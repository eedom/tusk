...
class Maze {
  ...
  private void init (int size) {
    if (size < MIN_SIZE) this.size = Maze.MIN_SIZE;
    else this.size = size;
  ...
  void generate () {
    ...
    for (int x = 0; x < this.size; x++)
      for (int y = 0; y < this.size; y++)
        if (r.nextDouble() > Maze.PERCENT_BLANKS) this.cells[x][y] = true;
    this.cells[0][0] = false;
    this.cells[this.size - 1][this.size - 1] = false;
  }
  
  boolean isWall (final Location location) {
    if (location.getX() < 0 || location.getX() > this.size - 1) return true;
    if (location.getY() < 0 || location.getY() > this.size - 1) return true;
  ...
  public void draw (Graphics g) {
    ...
    for (int x = 0; x < size; x++) {
      for (int y = 0; y < size; y++) {
        if (cells[x][y]) g.fillRect(x * cellWidth, y * cellHeight, cellWidth, cellHeight);
      }
    }
  }
}